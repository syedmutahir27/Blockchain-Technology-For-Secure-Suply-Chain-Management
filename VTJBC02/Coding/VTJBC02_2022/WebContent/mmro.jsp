				...
				<article>
					...
					<div class="inner">
						...
						%>
							...
							<th style="color: white;">Email</th><th style="color: white;">Domain</th>
							<th style="color: white;">Mobile</th></tr>
							<%
							String sql = "select * from mro where cid='"+session.getAttribute("uid")+"'";
							List<String> lt = Dao.getMro(sql);
							Iterator<String> itr = lt.iterator();
							while(itr.hasNext()){
								String id = itr.next();
							%>
							<tr><td><a href="mrepair.jsp?id=<%=id%>"><%=id %></a></td><td><%=itr.next() %></td>
							<td><%=itr.next() %></td><td><%=itr.next() %></td>
							<td><%=itr.next() %></td></tr>
							<%} %>
							</table>
							<a href="addmro.jsp">Add MRO</a>
						</header>
					</div>
				</article>
			</section>
